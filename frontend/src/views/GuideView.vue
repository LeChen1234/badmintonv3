  ...
  <div class="guide-page">
    ...
    <el-card shadow="never" class="section-card">
      ...
      <el-alert class="flow-tip" type="info" :closable="false"
        title="最短路径：上传 → 选采集场景并填必填项 → 选人物/动作 → 画框和修正姿态 → 保存并下一帧。带“可选”的高级字段按需展开。" />
    </el-card>
    ...
    <el-card shadow="never" class="section-card">
      ...
      <el-collapse accordion>
        <el-collapse-item title="1. 上传视频" name="upload">
          ...
          <p>默认均匀抽帧即可；只有需要压缩大量相似帧时，才展开“抽帧设置”启用动作筛选。筛选帧始终可按时间戳加回。</p>
        </el-collapse-item>
        <el-collapse-item title="2. 先选择正确的数据轨道" name="protocol">
          <p><strong>比赛/动作时序轨：</strong>必须记录来源链接或赛事编号；适合动作类别、阶段、受迫性、移动和战术，不能切换为精细动作质量轨。</p>
          <p><strong>手机训练质量轨：</strong>适合动作质量、完整动作序列和清晰可见的球拍接触细节。系统自动检查原视频帧率，还需记录自然训练/指定动作、喂球方式、重复组和桥接视角。</p>
        ...
        <el-collapse-item title="7. 四层击球事件" name="stroke-event">
          <p>连续片段按“来球情境—动作执行—击球后果—证据可见性”填写。比赛视频优先填写情境和后果，手机训练优先填写执行和接触。</p>
          <p>相同条件连续重复时才勾选“保存后保留本组条件”；否则系统保存后自动清空，避免上一动作的字段误带到下一动作。</p>
        </el-collapse-item>
      </el-collapse>
    </el-card>
...
const studentSteps = [
  ...
  '填写基础动作类型和是否为击球接触事件；画面清晰时才展开接触几何。',
  '点击“保存并下一帧”完成当前帧；系统在切换帧时也会保护未保存修改。',
  '完成全部帧后进行自核并提交审核。',
]
const expertSteps = [
  '进入“审核流程”，打开“体育专家判定队列”。',
